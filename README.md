...
| --- | --- | --- |
| [linear-tickets](./linear-tickets) | Connects to Linear's read-only MCP endpoint, lists the tickets assigned to you, and starts an agent with the ticket details and comments in its first prompt. Filter by workflow status, search, sort, then pick a project, base branch, provider and model before launching. | Paseo ≥ 0.8.0, Node.js ≥ 22 on the daemon host |
